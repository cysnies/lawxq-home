...
export const helloInit = () => {
  ...
  ElMessage({
    ...
    message: `<strong>${hello}</strong> 欢迎来到法律星球！`,
  });
};
...
export const siteDateStatistics = (startDate) => {
  ...
  if (differenceInYears >= 1) {
    return `本站已经创建了 ${Math.floor(differenceInYears)} 年 ${Math.floor(
      differenceInMonths % 12,
    )} 月 ${Math.round(differenceInDays % 30)} 天`;
  } else if (differenceInMonths >= 1) {
    return `本站已经创建了 ${Math.floor(differenceInMonths)} 月 ${Math.round(
      differenceInDays % 30,
    )} 天`;
  } else {
    return `本站已经创建了 ${Math.round(differenceInDays)} 天`;
  }
};
